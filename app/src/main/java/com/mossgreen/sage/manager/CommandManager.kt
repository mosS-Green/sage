...
class CommandManager(context: Context) {
    ...
    private val systemDefinitions = listOf(
        ...
        "ss" to "Capture complete accessibility screen context to SS Outputs log."
    )
    ...
    fun findCommand(text: String): Command? {
        ...
        // Transcription trigger — e.g., ?tr 01:23 or ?tr 0:45
        val trPrefix = "${prefix}tr "
        val trIdx = text.lastIndexOf(trPrefix)
        if (trIdx >= 0) {
            val durationPart = text.substring(trIdx + trPrefix.length).trim()
            if (durationPart.matches(Regex("\\d{1,2}:\\d{2}"))) {
                return Command("${trPrefix}$durationPart", "Transcribe voice note matching duration $durationPart", true)
            }
        }
        return null
    }
